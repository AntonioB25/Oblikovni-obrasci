...
#include <stdlib.h>
  ...
  void moveSquare(struct Square* square, int x, int y){
   ...
   void moveCircle(struct Circle* cirlce, int x, int y){
        cirlce->center_.x += x;
        cirlce->center_.y += y;
  }
...
void printPositionOfSquare(Square* square) {
  ...
  int main(){
        ...
        std::cout << "\nPrije pomaka:\n";
        printPositionOfSquare((struct Square*)shapes[1]);
        //moveShapes(shapes, 5, 1 , 8);                          // javlja gresku kod rhomba jer za njega nema move metode
        std::cout << "Nakon pomaka:\n";
        printPositionOfSquare((struct Square*) shapes[1]);
